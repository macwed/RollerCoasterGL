...
float gameplay::TrackComponent::approximateSForPoint_(const Spline& spline, const glm::vec3& p, float s0, float s1,
                                                     float ds)
{
...
void gameplay::TrackComponent::rebuildLUT_()
{
    spline_.rebuildArcLengthLUT(64);
}

void gameplay::TrackComponent::syncMetaWithSpline_()
{
    nodeMeta_.resize(spline_.nodeCount());
    edgeMeta_.resize(spline_.segmentCount());
}

void gameplay::TrackComponent::buildStationIntervals_()
{
    stations_.clear();
    if (spline_.segmentCount() == 0) return;

    const float L = spline_.totalLength();
    auto pushInterval = [&] (float a, float b)
    {
        if (a <= b) stations_.emplace_back(a, b);
        else
        {
    ...
    {
        const float auto& m = nodeMeta_[i];
        ...
        if (m.stationStart && stationL > 0.f)
        {
            ...
            if (sB <= L) pushInterval(sA, sB);
            else pushInterval(sA, std::fmod(sB, L));
        }
    }
    ...
    for (std::size_t i = 0; i + 1 < edgeMeta_.size(); ++i)
    {
        if (nodeMeta_[i].stationStart && nodeMeta_[i + 1].stationEnd)
        {
            ...
            float sB = spline_.isNodeOnCurve(i + 1) ? spline_.sAtNode(i + 1)
...
void gameplay::TrackComponent::rebuildRollKeys_()
{
    rollKeys_.clear();
    ...
    std::ranges::sort(rollKeys_,
                      [](const RollKey& a, const RollKey& b){ return a.s < b.s; });
    ...
    for (const auto& k : rollKeys_)
    {
        ...
        else merged.push_back(k);
    }
    rollKeys_.swap(merged);
}
    void gameplay::TrackComponent::buildFrames_()
{
    PathSampler sampler(spline_, edgeMeta_);
    ...
    frames_ = PTF::build(sampler, ds_, up_, cb);
}

//---------------------------------public API-----------------------------------------------

void gameplay::TrackComponent::rebuild()
{
    if (dirtySpline_)
    {
        rebuildLUT_();
...
bool gameplay::TrackComponent::isInStation(float s) const
{
    for (auto [a, b] : stations_)
    {
...
float gameplay::TrackComponent::stationEdgeFadeWeight (float s) const
{
    if (stations_.empty()) return 0.f;
    for (auto [a, b] : stations_)
    {
        if (s >= a - feather_ && s < a)
        {
...
float gameplay::TrackComponent::manualRollAtS (float s) const
{
...
glm::vec3 gameplay::TrackComponent::positionAtS(float s) const
{
    return spline_.getPositionAtS(s);
}

glm::vec3 gameplay::TrackComponent::tangentAtS(float s) const
{
    return glm::normalize(spline_.getTangentAtS(s));
}

Frame gameplay::TrackComponent::frameAtS(float s) const
{
    if (frames_.empty()) return { positionAtS(s), tangentAtS(s), {0.f, 1.f, 0.f}, {1.f, 0.f, 0.f}, s };

    if (s <= frames_.front().s) return frames_.front();
    if (s >= frames_.back().s) return frames_.back();

    auto it = std::upper_bound(frames_.begin(), frames_.end(), s,
        [](float v, const Frame& f) { return v < f.s; });
